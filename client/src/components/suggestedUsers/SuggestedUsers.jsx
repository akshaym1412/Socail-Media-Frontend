...
const SuggestedUsers = () => {
  ...
  useEffect(() => {
    const fetchSuggestedUsers = async() => {
      try {
         const res = await fetch(`https://social-media-psi-nine.vercel.app/user/find/suggestedUsers`, {
          headers: {
  ...
  const toggleFollow = async(id) => {
    try {
      await fetch(`https://social-media-psi-nine.vercel.app/user/toggleFollow/${id}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        },
        method: "PUT"
