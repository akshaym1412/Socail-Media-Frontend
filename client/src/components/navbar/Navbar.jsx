...
useEffect(() => {
  const fetchAllUsers = async() => {
    try {
      const res = await fetch(`https://social-media-psi-nine.vercel.app/user/findAll`)
...
const handleUpdateProfile = async(e) => {
  ...
  if(photo){
    ...
    await fetch(`https://social-media-psi-nine.vercel.app/upload/image`, {
      headers: {
  ...
  try {
    const res = await fetch(`https://social-media-psi-nine.vercel.app/user/updateUser/${user._id}`, {
      headers: {
            ...
            <div className={classes.navigation}>
                  ...
                  <div onClick={() => setSearchText("")} className={classes.allUsersContainer}>
                    {filteredUsers?.map((user) => (
                      <Link to={`/profileDetail/${user._id}`} key={user._id} onClick={() => setShowMobileNav(false)}>
                        <img src={user?.photo ? `https://social-media-psi-nine.vercel.app/images/${user.photo}` : man} />
                        <div className={classes.userData}>
                          <span>{user?.username}</span>
                          <span>{user?.bio?.slice(0, 10)}</span>
                        </div>
              ...
              <div className={classes.right}>
                ...
                <img
                  ...
                  src={user?.profileImg ? `https://social-media-psi-nine.vercel.app/images/${user?.profileImg}` : man}
