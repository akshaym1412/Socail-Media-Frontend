...
const ProfileDetail = () => {
  ...
  useEffect(() => {
    const fetchProfile = async() => {
       try {
        const res = await fetch(`https://social-media-psi-nine.vercel.app/user/find/${id}`, {
          headers: {
  ...
  useEffect(() => {
    const fetchProfilePosts = async() => {
      try {
        const res = await fetch(`https://social-media-psi-nine.vercel.app/post/find/userposts/${id}`)
  ...
  const handleFollowFunction = async() => {
    try {
      await fetch(`https://social-media-psi-nine.vercel.app/user/toggleFollow/${profile?._id}`, {
        headers: {
          'Authorization': `Bearer ${token}`
        },
        method: "PUT"
      ...
      <div className={classes.wrapper}>
        <div className={classes.top}>
          <div className={classes.topLeftSide}>
            <img 
             src={profile?.profileImg ? `https://social-media-psi-nine.vercel.app/images/${profile?.profileImg}` : man}
